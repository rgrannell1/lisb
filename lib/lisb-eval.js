...
const lEval = ( function () {
	...
	*/
	...
	evalForm.let = function (EXPR, SCOPE) {
		...
		const VALNAME = _leval(EXPR[1], function (val) {
			return isValname(val) || is.array(val)
		}, SCOPE)

		const VAL = _levalToPrimitive(EXPR[2], SCOPE)

		console.log(catchCall(EXPR))
		console.log(catchCall(SCOPE))
		console.log('------------------------------')

		console.log('need to evaluate second argument.')
		...
		if (is.array(VALNAME)) {
			...
			for (var ith = 1; ith < VAL.length; ith++) {
				...
				var innerLet = ['let', VALNAME[ith], VAL[ith]]
	...
	*/
	...
	evalForm.callProc = function (EXPR, SCOPE) {
		...
		if (PROC.evalArgs === true) {

			var PROCARGS = mapRest(function (expr) {
				return _levalToPrimitive(expr, SCOPE)
			}, EXPR)

		} else if (PROC.evalArgs === false) {
			...
			throw 'evalArgs was not defined.'
		}
