...
const stdlib       = require('./lisb-stdlib')
const macros       = require('./lisb-macros')
...
const Scope = function (parent) {
	...
	var scope = Object.create(stdlib)
...
const lEval = ( function () {
	...
	*/
	...
	/*
		...
		The rlet special form performs destructuring assignment.
	*/
	...
	*/
	...
	/*
		...
		The thread special forms applies functions one after another to
		an initial value.
	*/

	evalForm['->'] = function (EXPR, SCOPE) {

		checkNotUndefined(EXPR)

		var result = _levalToPrimitive(EXPR[1], SCOPE)

		for (var ith = 2; ith < EXPR.length; ith++) {
			result = _levalToPrimitive(EXPR[ith], SCOPE)(result)
		}
	...
	*/
	...
	/*
		...
		The mr special form is similar to the function special form, but it doesn't
		even evaluate its arguments.
	*/
	...
	*/
	...
	/*
		...
		The eval special form takes a data-structure and evaluates it to
		yield its result.
	*/
	...
	*/
	...
	/*
		...
		Unquote acquires a special meaning within the quasiquote macro's
		scope; unquoted expressions are evaluated within quasiquote's scope.
	*/
	...
	*/
	...
	/*
		...
		Create a list, a wrapper for JavaScript arrays. This form is
	...
	evalForm.callProc = function (EXPR, SCOPE) {
		...
		return indicesOf(PROCARGS)
			.reduce(function (curried, ith) {
				...
				if ( !is.undefined(typeParam[param]) ) {
	...
	*/
	...
	return function (EXPR, SCOPE, opts) {
		...
		return _levalToPrimitive(
			EXPR, SCOPE? SCOPE: Scope({}), opts)
	}
...
module.exports = {
	lEval: function (EXPR, opts) {
		return lEval(macros.concat([EXPR]), undefined, opts)
	}
}
