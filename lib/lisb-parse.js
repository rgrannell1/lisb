...
const isVARNAME = function (EXPR) {
	return is.string(EXPR) && EXPR.charAt(0)
}
...
const lisbEval = function (EXPR) {
	...
	const action = {
		...
		'begin': function (EXPR) {
			...
			EXPR.slice(1).map(function (expr) {
				leval(expr, isPrimitive)
			})

		},
		'callproc' : function (EXPR) {

			const PROC = lookup(leval(EXPR[0], isVARNAME))
