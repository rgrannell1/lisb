...
stdlib[':is'] = function (val0) {
	return function (val1) {
		...
		var val0 = unlist(val0)
		var val1 = unlist(val1)

		const val0Type = Object.prototype.toString.call(val0)
		const val1Type = Object.prototype.toString.call(val1)

		if (val0Type !== val1Type) {
			return false
		}
		if (val0Type === '[object Number]') {
			return val0 === val1
		} else if (val0Type === '[object String]') {
			return val0 === val1
		} else if (val0Type === '[object Function]') {
			throw ':is: unimplemented'
		} else if (val0Type === '[object Array]') {
			...
			for (var ith = 0; ith < val0Len; ith++) {
				allEqual = allEqual && stdlib[':is'](val0[ith])(val1[ith])
			}

			return allEqual
		} else {
			throw ':is, no matching subdef'
		}
